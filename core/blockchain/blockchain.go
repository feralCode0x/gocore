...
import (
	...
	transactions "gocore/core/transactions"
)
...
// ContinueBlockchain abre una base de datos de blockchain existente
func ContinueBlockchain(nodeID string) *Blockchain {
	dbFileWithNode := fmt.Sprintf(dbFile, nodeID)
    
    // Verificamos si el archivo de base de datos existe
	if _, err := os.Stat(dbFileWithNode); os.IsNotExist(err) {
		fmt.Println("No existing blockchain found. Create one first.")
		os.Exit(1)
	}

	var tip []byte
	db, err := bolt.Open(dbFileWithNode, 0600, nil)
	if err != nil {
		panic(err)
	}
	...
	err = db.Update(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(blocksBucket))
		tip = b.Get([]byte("l")) // "l" es la llave para el último hash (last)
